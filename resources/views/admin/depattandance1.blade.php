...
<div class="container-fluid px-5">
    ...
    <div class="row">
        <div class="col-sm-4 mb-1 mb-sm-0">
            <form action="{{url('get-dep-attendance')}}" id="myForm" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <span style="display: flex;">
                ...
                <select id="name" name="name" style="border: 1px solid #bfbfbf;" class="form-control select.custom-select" required>
                    @foreach($empname as $value)
                    <option value="{{ $value['code'] }}">{{ $value['name'] }}</option>
                    @endforeach
                </select>
                <a id="submit" style="font-size: small; cursor: pointer; background: linear-gradient(14deg, #1761fd 0%, rgba(23, 97, 253, 0.6)) !important; border: none;" class="btn text-white ModelBtn ml-2 py-0 px-2"><i style="font-size: 20px;" class="mdi mdi-send"></i></a>
            </span>
            </form>
        </div>
    </div>
